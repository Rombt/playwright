...
export default {
  create(deps: ISourceDependencies): ISource<ICollectProductPhotosTask> {
    return new PageImageSourceSrm(deps.flowRunner);
  },
};

class PageImageSourceSrm implements ISource<ICollectProductPhotosTask> {
  constructor(
    private flowRunner: IFlowRunner, // private logger: ILogger,
  ) {}

  supports(task: ICollectProductPhotosTask): boolean {
    return task.metadata.target_website === 'https://srmknives.com.ua/search?search={{sku_prod}}';
  }
  ...
  async execute(ctx: IExecutionContext<ICollectProductPhotosTask>): Promise<IWorkerResult> {
    ...
    ctx.logger?.debug('Processing of the PageImageSourceSrm is finished', {
      component: 'PageImageSourceSrm',
      method: 'execute()',
