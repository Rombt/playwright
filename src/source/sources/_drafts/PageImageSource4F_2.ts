...
export default {
  create(deps: ISourceDependencies): ISource<ICollectProductPhotosTask> {
    ...
    return new PageImageSource4F_2(deps.flowRunner);
  },
};

class PageImageSource4F_2 implements ISource<ICollectProductPhotosTask> {
  constructor(
  ...
  supports(task: ICollectProductPhotosTask): boolean {
    return (
      ...
      'https://fanaticsport.ua/ua/catalogsearch/result/?q={{sku_prod}}'
    );
  }
  ...
  async execute(ctx: IExecutionContext<ICollectProductPhotosTask>): Promise<IWorkerResult> {
    ...
    ctx.stepParams = new Map([
      [
        CheckSearchResultsStep,
        {
          ...
          linkSelector: 'div.images-container a.product-item-photo',
          emptySelector: 'div.message.notice div',
          emptySelectorText: ' На Ваш запит нічого не знайдено. ',
        },
      ],
      [CheckPageSkuStep, { pageSkuSelector: 'div.product.attribute.sku > div[itemprop="sku"]' }],
      [
        SearchGalleryStep,
        {
          gallerySelector: 'div.[data-gallery-role="gallery"]',
        },
      ],
      [
        CollectImgStep,
    ...
    ctx.logger?.debug('Processing of the PageImageSource4F_2 is finished', {
      component: 'PageImageSource4F_2',
      method: 'execute()',
